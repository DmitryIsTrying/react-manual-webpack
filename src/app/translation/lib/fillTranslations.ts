...
function fillTranslations(keys: Set<string>, locales: string[], mainLocale: string) {
  ...
  locales.forEach((locale) => {
    ...
    keysArray.forEach((key) => {
      ...
      translations[key] = locale === mainLocale ? key : `${locale.toUpperCase()}-${key}`;
    });

    // Создаём директорию, если её нет
    const dirPath = path.join(__dirname, "../../../../public", "locales", locale);
